...
function __element_dimensions( lmnt ) = __type( lmnt ) == DIVIDERS
                                        ? [ __div_frame_size( lmnt )[k_x],  __div_total_height( lmnt ) ]
                                            :  __type( lmnt ) == HEXBOX
                                            ?  __value( lmnt, HEXBOX_SIZE_DZ, default = [ 100, 10] )
...
///////////////////////
            ...
            if( m_lid_magnet_use_ring )
            {
                ...
                intersection() 
                {
                    ...
                    difference() 
                    {
                        ...
                        {
                            offsetXY = 0;
                            usableX = __lid_external_size(k_x) - magnetRingRadius;
                            ...
                            for(i = [0: m_lid_magnet_count_x - 1])
                                for(j = [0: m_lid_magnet_count_y - 1])
                                    ...
                                    {
                                        translate( [
                                                usableX * i / (m_lid_magnet_count_x-1) + magnetRingRadius / 2, 
                                                usableY * j / (m_lid_magnet_count_y-1),
                                                zOffSet - magnetRingHeight] )
                                            scale(
                                                [
                                                    i == 0 || i == m_lid_magnet_count_x - 1 ? 1.7 : 1,
        ...
        module MakeMagnetHoles(zOffSet = 0)
        {
            ...
            spacingX = usableX / (m_lid_magnet_count_x - 1);
            spacingY = usableY / (m_lid_magnet_count_y - 1);
            echo(usableX);
            echo(spacingX);
            for(i = [0: m_lid_magnet_count_x - 1])
                for(j = [0: m_lid_magnet_count_y - 1])
                    ...
                    {
                        translate( [
                                spacingX * i + m_lid_magnet_radius + m_lid_magnet_margin, 
                                spacingY * j / (m_lid_magnet_count_y-1) + m_lid_magnet_radius + m_lid_magnet_margin,
        ...
        module MakeLid() 
        {
            ...
            module MakeLidSurface()
            {
                ...
                if( m_lid_pattern_padding > 0 )
                {
                    difference() 
                    {
                        cube([__lid_external_size( k_x ), __lid_external_size( k_y ), m_lid_wall_height]);
                        translate([m_lid_pattern_padding ,m_lid_pattern_padding,0])
                            cube([__lid_external_size( k_x ) - m_lid_pattern_padding * 2, __lid_external_size( k_y ) - m_lid_pattern_padding * 2, m_lid_wall_height]);
                        if( m_lid_magnet )
                        {
                ...
                intersection() 
                {
                    ...
                    difference()
                    {
                        ...
                        MakeMesh( thickness = m_lid_wall_height );
...
module MakeHexBox( box )
{
    ...
    m_box_size = ( g_b_fit_test && m_box_has_lid ) ?
        [   __element_dimensions( box )[ k_hex_d ], 
        __element_dimensions( box )[ k_hex_z ] + __lid_external_size( k_z )
        ] :
        __element_dimensions( box );
    ...
    else
    {
        ...
        if ( g_b_print_box )
        {
            ...
            // Add lid holder at the very end to prevent components from chopping into it
            MakeLayer( layer = "lid_holder" );
    ...
    module MakeLayer( component, layer = "" )
    {
        ...
        if ( __is_component_enabled() )
        {
            ...
            else if ( m_is_lid_subtractions )
            {
                ...
                if ( m_lid_inset && m_box_is_stackable )
                {
                    difference()
                    {
                        ...
                        MirrorAboutPoint( v=[0,0,1], pt=[0,0,__lid_external_size(k_z)/2])
                            MakeHexLidBase_Inset( tolerance = g_tolerance, tolerance_detent_pos = g_tolerance_detent_pos, omit_detents = !m_lid_inset );
            ...
            else if ( m_is_lid_holder )
            {
                MakeHexBoxInsetLidHolder();
            }
        ...
        module MakeHexLidBase_Inset( tolerance = 0, tolerance_detent_pos = 0, omit_detents = false )
        {
            m_epsilon = 0.001;
            translate([m_outer_diameter / 2.0, (m_outer_diameter / 2.0) * sin(60), 0])
            {
                difference() 
                {
                    ...
                    difference()
                    {
                        cylinder($fn = 6, d = m_outer_diameter, __lid_external_size( k_z ));
                        hull () {
                            cylinder($fn = 6, d = m_outer_diameter, m_epsilon);
                            cylinder($fn = 6, d = m_inner_diameter - (tolerance / sin(60)), __lid_external_size( k_z ) / 2.0);
                        }
                        translate([0, 0, -m_epsilon])
                        {
                            cylinder($fn = 6, d = (m_outer_diameter + m_inner_diameter) / 2.0 - (tolerance / sin(60)), __lid_external_size( k_z ) + 2 * m_epsilon);
                        }
                    }
                }
            }
        ...
        module MakeHexBoxInsetLidHolder()
        {
            if ( m_box_has_lid || m_box_is_stackable )
            {
                difference() {
            ...
            module Helper__BuildLid()
            {
                ...
                {             
                    if ( m_lid_inset )
                    {
                        ...
                        difference( tolerance = g_tolerance )
                        {
                            MakeHexLidBase_Inset( tolerance = g_tolerance, tolerance_detent_pos = g_tolerance_detent_pos );

                            translate([m_outer_diameter / 2.0, (m_outer_diameter / 2.0) * sin(60), 0])
                                cylinder($fn = 6, d = m_inner_diameter, __lid_external_size( k_z ));
                        }
        ...
        module MakeLidTabs( mod = 0.0, square = false )
        {
            module MakeLidTab( mod )
            {
                ...
                // square part
                translate( [ -x/2, -m_epsilon, 0])
                    cube( [ x, y, z  ], center = false );

                if ( square )
                {
                    translate( [ -x/2, -m_epsilon, z ])
                        cube( [ x, y, y + m_lid_wall_height ], center = false );
                }
                else
                    // prism part
            ...
            for ( i = [ 0 : 5 ] )
                RotateAboutPoint( 60 * i, [0,0,1], [m_outer_diameter / 2.0, (m_outer_diameter / 2.0) * sin(60), 0] )
                    translate([m_outer_diameter / 2.0, 0, 0])
                        MakeLidTab( mod );
